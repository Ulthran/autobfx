...
def qc_flow(project_fp: Path, config: Config, samples: dict[str, Path] = {}) -> list[Path]:
    samples_list = samples if samples else gather_samples(get_input_fp(Path(config.flows["trimmomatic"].input), project_fp), config.paired_end)
    results = {sample_name: {} for sample_name in samples_list.keys()}

    async def qc_for_sample(sample_name, r1):
        results[sample_name]["trimmomatic"] = trimmomatic_flow(project_fp, config, samples={sample_name: r1})
        results[sample_name]["heyfastq"] = heyfastq_flow(project_fp, config, samples={sample_name: r1})
        results[sample_name]["fastqc"] = fastqc_flow(project_fp, config, samples={sample_name: r1})

    async def submit_flows():
        await asyncio.gather(*[qc_for_sample(sample_name, r1) for sample_name, r1 in samples_list.items()])
...
if __name__ == "__main__":
    ...
    run_deployment(f"{NAME}/{NAME}_deployment", parameters={"project_fp": Path(args.project_fp).absolute(), "config": config, "samples": samples})
