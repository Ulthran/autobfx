...
def decontam_flow(
    ...
    hosts_list = [x.resolve() for x in Path(config.flows[NAME].parameters["hosts"]).glob("*.fasta")]

    # Setup
    build_host_index_input_fp, build_host_index_output_fp, build_host_index_log_fp = setup_step(
        project_fp, config, "build_host_index"
    )
    align_to_host_input_fp, align_to_host_output_fp, align_to_host_log_fp = setup_step(
        project_fp, config, "align_to_host"
    )
    filter_host_reads_input_fp, filter_host_reads_output_fp, filter_host_reads_log_fp = setup_step(
        project_fp, config, "filter_host_reads"
    )
    preprocess_report_input_fp, preprocess_report_output_fp, preprocess_report_log_fp = setup_step(
        project_fp, config, "preprocess_report"
    )
    ...
    align_to_host_results = {(sample_name, host_fp.stem): None for sample_name in samples_list.keys() for host_fp in hosts_list}
    for sample_name, r1 in samples_list.items():
        with tags(sample_name):
            with tags("align_to_host"):
                for host_fp in hosts_list:
                    with tags(host_fp.stem):
                        align_to_host_results[(sample_name, host_fp.stem)] = run_align_to_host.submit(
                            input_fp=r1,
                            output_fp=align_to_host_output_fp / f"{sample_name}.sam",
                            ...
                            **config.flows["align_to_host"].parameters,
                            wait_for=[build_host_index_results[host_fp.stem]],
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--samples", nargs="*", default=[], help="Specific samples to run (provide full sample names e.g. 'sample1_R1.fastq.gz' as they appear in the input directory for this flow)"
    )
